'use strict';
    ...
    reader.onload = function(evt) {
      ...
      for (i = 0; i < trksegs.length; i++) {
        ...
        for (j = 0; j < trkpts.length; j++) {
        	rg2.gpstrack.lat.push(trkpts[j].getAttribute('lat'));
        	rg2.gpstrack.lon.push(trkpts[j].getAttribute('lon'));
        	timestring = trkpts[j].childNodes[3].textContent;
        	rg2.gpstrack.time.push(rg2.gpstrack.getSecsFromTrackpoint(timestring)); 
        }
      }
      rg2.gpstrack.processGPSTrack();
  ...
  fitTrackInsideCourse: function() {
    ...
    // extra offset to put start of track at start location 
		...
		var deltaX = minControlX - (this.routeData.x[0] - draw.controlx[0]); 
		var deltaY = minControlY - (this.routeData.y[0] - draw.controly[0]);
